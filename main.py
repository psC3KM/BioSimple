...
from export import export_to_excel, export_to_word, export_to_csv, export_to_json
import re
...
def add_sample_menu(manager):
    ...
    try:
        ...
        date = input('Дата сбора: (ДД.ММ.ГГГГ)')
        if not re.match(r'\d{2}\.\d{2}\.\d{4}', date):
            print('Ошибка: неверный формат даты')
            return

        location = input('Место сбора')
        notes = input('Примечение (необязательно): ')
...
def search_sample_menu(manager):
    ...
    results = manager.search_samples(**filters)
